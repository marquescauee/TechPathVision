...
export const getRoadmapsRequest = async (token: string, email: string): Promise<string> => {
  const response = await fetch(`${BASE_BACKEND_URL}/get-roadmaps`, {
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Token ${token}`
    },
  ...
  if (!response.ok) {
    return 'Falha ao recuperar roadmaps.'
  }
