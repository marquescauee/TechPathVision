...
def map_profile(request):
    ...
    jsonExample: Dict[str, Any] = [
      ...
      },
      {
        ...
        "shortDescription": "O desenvolvimento de sistemas envolve criar soluções complexas que atendam às necessidades dos usuários, utilizando skills em diversas áreas como hardware, software e inteligência artificial.",
        "longDescription1": "O desenvolvimento de sistemas é um campo amplo que requer skills em diversas áreas, incluindo hardware, software e inteligência artificial. Os desenvolvedores precisam criar soluções que sejam escaláveis, seguras e fáceis de usar, utilizando tecnologias como sistemas operacionais, redes de computadores e bancos de dados.",
    ...
    if serializer.is_valid():
        model = 'qwen2.5:3b'
        question = f'Mapeie NO MÍNIMO 5 áreas compatíveis com o perfil de usuário informado. Você deve retornar 5 áreas. Para cada área, você deve estruturar um JSON que contenha: título da área, descrição resumida e descrição detalhada. A descrição resumida deve ter algo entre 5 e 10 linhas. A descrição detalhada deve ser muito bem descrita, informando diversos detalhes da área. Responda apenas o array de JSON, sem textos adicionais antes ou depois. NÃO UTILIZE ASPAS SIMPLES NO JSON, OU SEJA, SEMPRE UTILIZE ASPAS DUPLAS. Os atributos são: {profile}. Para cada campo do JSON, você deve trazer a resposta em texto corrido. Não repita áreas com o mesmo título. Foque em trazer títulos que sejam noticiados na mídia ou em artigos. Por exemplo, desenvolvimento de aplicações envolve muita coisa. Nesse caso, você deve quebrar essa área em várias, como Front-End, Back-End, Administrador de Banco de Dados, Arquiteto de Software e etc. No resumo, escreva em texto corrido sem utilizar tantas frases curtas. Articule um pouco mais, com conectivos, conjunções, etc. O JSON pode ter as chaves "title", "shortDescription" e "longDescription1", "longDescription2" e etc. No fim, deve ter um campo "skills", em que você vai elencar as skills necessárias para aquela área. Diversifique ao máximo as áreas mapeadas, evitando informar áreas com características semelhantes. Utilize esse JSON como exemplo: {jsonExample}'
        ...
        json_response = json.loads(response['message']['content'].replace("'", '"'))
